...
export default function Comment({ slug }) {
  ...
  useEffect(() => {
    const myFunc = async () => {
      if (slug === undefined || db === undefined) return;
      initializeFirebase();

      const dataRef = doc(db, "comments", slug);
      const dataSnap = await getDoc(dataRef);
      if (dataSnap.exists()) {
        setCommentData(dataSnap.data().comments);
        setRecentComment(dataSnap.data().comments?.slice(-3));
      } else {
